...
class File_model extends CI_Model
{
    ...
    public function upload_image_session()
    {
        ...
        if (!empty($temp_path)) {
            ...
            $emarket_images = $this->get_sess_product_images_array();
            $item = new stdClass();
            ...
            $item->is_main = 0;
            $item->file_time = time();
            array_push($emarket_images, $item);
            $this->set_sess_product_images_array($emarket_images);
            $this->upload_model->delete_temp_image($temp_path);
        }
    }
    ...
    {
        $emarket_images = $this->get_sess_product_images_array();
        if (!empty($emarket_images)) {
            foreach ($emarket_images as $emarket_image) {
                if (!empty($emarket_image)) {
                    $image_storage = "local";
                    $this->load->model('upload_model');
                    ...
                    if ($this->storage_settings->storage == "aws_s3") {
                        ...
                        $this->load->model("aws_model");
                        //move default image
                        $this->aws_model->put_product_object($directory . $emarket_image->img_default, FCPATH . "uploads/temp/" . $emarket_image->img_default);
                        delete_file_from_server("uploads/temp/" . $emarket_image->img_default);
                        //move big image
                        $this->aws_model->put_product_object($directory . $emarket_image->img_big, FCPATH . "uploads/temp/" . $emarket_image->img_big);
                        delete_file_from_server("uploads/temp/" . $emarket_image->img_big);
                        //move small image
                        $this->aws_model->put_product_object($directory . $emarket_image->img_small, FCPATH . "uploads/temp/" . $emarket_image->img_small);
                        delete_file_from_server("uploads/temp/" . $emarket_image->img_small);
                    } else {
                        //move default image
                        copy(FCPATH . "uploads/temp/" . $emarket_image->img_default, FCPATH . "uploads/images/" . $directory . $emarket_image->img_default);
                        delete_file_from_server("uploads/temp/" . $emarket_image->img_default);
                        //move big image
                        copy(FCPATH . "uploads/temp/" . $emarket_image->img_big, FCPATH . "uploads/images/" . $directory . $emarket_image->img_big);
                        delete_file_from_server("uploads/temp/" . $emarket_image->img_big);
                        //move small image
                        copy(FCPATH . "uploads/temp/" . $emarket_image->img_small, FCPATH . "uploads/images/" . $directory . $emarket_image->img_small);
                        delete_file_from_server("uploads/temp/" . $emarket_image->img_small);
                    }
                    ...
                    $data = array(
                        'product_id' => $product_id,
                        'image_default' => $directory . $emarket_image->img_default,
                        'image_big' => $directory . $emarket_image->img_big,
                        'image_small' => $directory . $emarket_image->img_small,
                        'is_main' => $emarket_image->is_main,
    ...
    public function set_sess_product_images_array($emarket_images)
    {
        $this->session->set_userdata('emarket_product_images', $emarket_images);
    }
    ...
    public function set_sess_image_main($file_id)
    {
        ...
        if (!empty($emarket_images)) {
            foreach ($emarket_images as $emarket_image) {
                if ($emarket_image->file_id == $file_id) {
                    $emarket_image->is_main = 1;
                } else {
                    $emarket_image->is_main = 0;
                }
            }
        }
        $this->set_sess_product_images_array($emarket_images);
    ...
    {
        $emarket_images = array();
        if (!empty($this->session->userdata('emarket_product_images'))) {
            $emarket_images = $this->session->userdata('emarket_product_images');
        }
        if (!empty($emarket_images)) {
            usort($emarket_images, function ($a, $b) {
                if ($a->file_time == $b->file_time) return 0;
                return $a->file_time < $b->file_time ? 1 : -1;
            });
        }
        return $emarket_images;
    }
    ...
    {
        if (!empty($this->session->userdata('emarket_product_images'))) {
            $this->session->unset_userdata('emarket_product_images');
        }
    }
        ...
        if (!empty($emarket_images)) {
            foreach ($emarket_images as $emarket_image) {
                if ($emarket_image->file_id == $file_id) {
                    ...
                    $item = new stdClass();
                    $item->img_default = $emarket_image->img_default;
                    $item->img_big = $emarket_image->img_big;
                    $item->img_small = $emarket_image->img_small;
                    $item->file_id = $emarket_image->file_id;
                    $item->is_main = $emarket_image->is_main;
                    $item->file_time = $emarket_image->file_time;
                    array_push($emarket_images_new, $item);
                }
            }
        }
        unset($emarket_images);
        $this->set_sess_product_images_array($emarket_images_new);
    }
