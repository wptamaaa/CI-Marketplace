...
class Auth_model extends CI_Model
{
    ...
    public function login()
    {
        ...
        if (!empty($user)) {
            ...
            //set user data
            $user_data = array(
                'emarket_sess_user_id' => $user->id,
                'emarket_sess_user_email' => $user->email,
                'emarket_sess_user_role' => $user->role,
                'emarket_sess_logged_in' => true,
                'emarket_sess_app_key' => $this->config->item('app_key'),
            );
    ...
    {
        //unset user data
        $this->session->unset_userdata('emarket_sess_user_id');
        $this->session->unset_userdata('emarket_sess_user_email');
        $this->session->unset_userdata('emarket_sess_user_role');
        $this->session->unset_userdata('emarket_sess_logged_in');
        $this->session->unset_userdata('emarket_sess_app_key');
    }
    ...
    public function is_logged_in()
    {
        ...
        if ($this->session->userdata('emarket_sess_logged_in') == true && $this->session->userdata('emarket_sess_app_key') == $this->config->item('app_key')) {
            $user = $this->get_user($this->session->userdata('emarket_sess_user_id'));
            if (!empty($user)) {
                if ($user->banned == 0) {
                    return true;
                }
    ...
    public function get_logged_user()
    {
        if ($this->is_logged_in()) {
            $user_id = $this->session->userdata('emarket_sess_user_id');
