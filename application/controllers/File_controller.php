...
class File_controller extends Home_Core_Controller
{
    ...
    public function get_sess_uploaded_image()
    {
        ...
        $emarket_images = $this->file_model->get_sess_product_images_array();
        if (!empty($emarket_images)) {
            foreach ($emarket_images as $emarket_image) {
                if ($emarket_image->file_id == $file_id) {
                    echo '<img src="' . base_url() . "uploads/temp/" . $emarket_image->img_small . '" alt="">' .
                        '<a href="javascript:void(0)" class="btn-img-delete btn-delete-product-img-session" data-file-id="' . $emarket_image->file_id . '"><i class="icon-close"></i></a>' .
                        '<a href="javascript:void(0)" class="btn btn-xs btn-secondary btn-is-image-main btn-set-image-main-session" data-file-id="' . $emarket_image->file_id . '">' . trans("main") . '</a>';
                    break;
                }
            }
        }
